...
def compute_metrics(state: dict[str, Any]) -> dict[str, float]:
    ...
    gene_bins = np.floor(gene[alive] * 10.0)
    diversity = np.unique(gene_bins).size / 10.0
    lineage_concentration = float(counts.max() / max(1, live_cells))
    return {
        ...
        "gene_variance": float(np.var(gene[alive])),
        "mean_charge": float(charge[alive].mean()),
        "mutability_mean": float(mutability[alive].mean()),
        ...
        "death_rate": float(state["last_deaths"] / max(1, total_cells)),
        "budget_entropy": float(state.get("last_budget_entropy", 0.0)),
        "budget_utilization": float(state.get("last_budget_utilization", 1.0)),
    }
